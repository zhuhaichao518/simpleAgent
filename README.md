...
### AI 现写定制小应用（15-30 秒，含真后端能力）

- 🔥 **「做一个 AI 故事书：输入主角和场景，AI 现写一段睡前故事并朗读」** —— 调 `LG.llm` + `LG.tts`
- 🔥 **「做一个产品经理 AI 模拟面试官，5 轮问答最后给评分」** —— 多轮 `LG.llm` 对话
- 🔥 **「做一个心情日记本，记录情绪+AI 评论+持久化」** —— `LG.llm` + `LG.kv`
- 「BMI 计算器」「贪吃蛇游戏」「打字练习+WPM」 —— 纯前端
- 「塔罗占卜：抽 3 张牌，AI 解读今日运势」 —— `LG.llm`

Agent 会把生成的小应用直接嵌入对话流：

- **点击交互**：所有应用都是可玩的真实工具，不是 demo
- **⛶ 进入全屏**：像微信小程序一样满屏体验
- **📦 我的应用**：自动收藏，点开可重复使用
- **编辑/分享**：点「编辑」改 JSON 配置，点「分享」复制链接

继续追加「把第二个换成米线」「再加一项寿司」即触发 `edit_app`。

### LG SDK（custom 应用可用的运行时 API）

LLM 生成的 HTML 在 iframe 里自动获得 `window.LG`：

```js
await LG.llm("用一句俏皮话夸夸我");      // 调后端 LLM
await LG.tts("Hello", { lang: "en" });   // 朗读
await LG.vibrate([100, 50, 100]);        // 振动节奏
await LG.kv.set("score", 99);            // 持久化
await LG.kv.get("score");                // → 99
await LG.toast("已保存");                // 父页面 toast
```

这是我们追平灵光闪应用「调用 AI 后端」核心能力的关键。详见 `client/src/sdk/lgBridge.js` 与 `AGENTS.md`。
